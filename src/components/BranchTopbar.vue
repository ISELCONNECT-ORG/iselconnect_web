...
const fetchBranchNotifications = async () => {
  // Get current session user to filter strictly for this branch account[cite: 2]
  const {
    data: { user },
  ...
  if (user && user.id) {
    // Filters specifically for notifications assigned to this branch user or broadcasted globally (null)[cite: 2]
