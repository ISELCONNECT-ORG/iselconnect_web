<template>
  <div class="dashboard-root">
    ...
    <main class="main-content">
      <!-- Main Profile Content (Centered with HomeBackground and fixed banner filename) -->
          ...
          <div class="card-body">
            ...
            <div class="form-section">
              ...
              <div class="input-group">
                <label>ROLE</label>
                <div class="input-wrapper">
                  <span class="input-text">{{ roleName }}</span>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="18"
                    height="18"
            ...
            <div class="status-section">
              <div class="status-card">
                ...
                <p class="status-desc">
                  This {{ roleName.toLowerCase() }} is authenticated and currently holds system
                  access permissions based on its active state.
                </p>
              </div>
            </div>
          </div>
...
// Dynamically map role_id from the database to proper titles
const roleName = computed(() => {
  switch (Number(profile.value.role_id)) {
    case 8:
...
const formattedName = computed(() => {
  if (!profile.value.first_name) return 'Super Admin'
...
const fetchProfile = async () => {
  ...
  // Automatically set is_active to TRUE in the database when online/logged in
  await supabase.from('users').update({ is_active: true }).eq('email', user.email)

  // Query user profile data
  const { data, error: dbError } = await supabase
    .from('users')
    .select('first_name, middle_name, last_name, is_active, role_id')
    .eq('email', user.email)
    .maybeSingle()
...
/* Image Banner using corrected filename bannerdashboard.jpg */
.card-banner {
  position: relative;
...
/* Subtle gradient overlay to ensure text readability over the background image */
.banner-overlay {
  position: absolute;
  top: 0;
  left: 0;
